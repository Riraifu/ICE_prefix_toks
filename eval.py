...
def PPL_new_target(
    ...
    sampled_token_ids = tok(prompt[0] + target_new, return_tensors="pt", padding=True, truncation=True)["input_ids"].to(device)
    with torch.no_grad():
...
def PPL_new(
    ...
    generation_sentence_ids = tok(generation_sentence, return_tensors="pt", padding=True, truncation=True)["input_ids"].to(device)
    ...
    with torch.no_grad():
        ...
        logits = outputs['logits']
        log_probs = torch.gather(F.softmax(logits, dim=-1).log2(), -1, generation_sentence_ids.unsqueeze(-1)).squeeze(-1)
...
from easyeditor.prefix_editor.modeling_gpt2 import GPT2LMHeadModel
from easyeditor.prefix_editor.modeling_llama import LlamaForCausalLM
