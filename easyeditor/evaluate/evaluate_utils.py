...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False):
    ...
    prompt_target_tok = tok(prompt_target,padding=True,truncation=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt",).to(f"cuda:{device}")
    prompt_tok = tok(prompts,padding=True,truncation=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt",)
...
# 重载 me
def test_prediction_acc2(embd_list, model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False,index=None,index2=None,vector_base=[],metric_type=None,start_end=[]):
    ...
    prefix_len = 1
    # prompt_ids = tok(prompts,padding=True,truncation=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt")['input_ids']
    prompt_ids = tok(prompts,padding=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt").to(device)['input_ids']
    prompt_target_ids = tok(prompt_target,padding=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt",).to(device)['input_ids']
    batchsize = prompt_target_ids.shape[0]
    PAD = torch.tensor(tok.pad_token_id).to(device).repeat(batchsize,prefix_len)
    prompt_ids = torch.cat((PAD,prompt_ids),dim=1)
    prompt_target_ids = torch.cat((PAD,prompt_target_ids),dim=1)
    
    # prompt_target_ids = tok(prompt_target,padding=True,truncation=True,max_length=max(hparams.max_length, max_prompt_len),return_tensors="pt",).to(f"cuda:{device}")['input_ids']
    # prompt_target_ids = tok(prompt_target,return_tensors="pt",padding='max_length',max_length=35).to(f"cuda:{device}")['input_ids']


    num_prompt_toks = [int((i != tok.pad_token_id).sum()+prefix_len) for i in prompt_ids]
    num_pad_toks = [int((i == tok.pad_token_id).sum()-prefix_len) for i in prompt_target_ids.cpu()]
    prompt_len = [x+y for x,y in zip(num_pad_toks,num_prompt_toks)]
    
    # print(f"prompt_ids: {prompt_ids}")
    # print(f"prompt_ids: {prompt_ids.shape}")
    # print(f"prompt_target_ids: {prompt_target_ids}")
    # print(f"prompt_target_ids: {prompt_target_ids.shape}")
    with torch.no_grad():
        # outputs = model(**prompt_target_tok)
        ...
        subject_attn_token = []
        from ..prefix_editor.utils  import register_hook,register_attn_hook,register_midattn_hook,register_subject_hook
        handle = register_hook(model,prompt_ids,hparams,query_last_ffn_token)
        handle2 = register_attn_hook(model,prompt_ids,hparams,query_last_attn_token)
        handle3 = register_midattn_hook(model,prompt_ids,prompt_target_ids,hparams,query_mid_attn_token)
        handle4 = register_subject_hook(model,prompt_ids,hparams,subject_attn_token,start_end)
        # if locality==False:handle4 = register_subject_hook(model,prompt_ids,hparams,subject_attn_token,start_end)
        # handle,handle2 = register_tf_hook(model,prompt_ids,hparams,query_last_token)
        
        dist,indices= None,None
        if len(start_end)>0:
            if index is not None:
                out = model(input_ids=prompt_ids,output_attentions=True)
                # out = model(input_ids=prompt_ids.to(device),past_key_values=PAD_past_kv,output_attentions=True)
                ...
                # print(f"query_last_attn_token: {query_last_attn_token}")
                # print(f"subject_attn_token: {subject_attn_token}")
                query_last_token.append(torch.cat((query_last_attn_token[0]*0,subject_attn_token[0]*1),dim=-1))
                vector = query_last_token[0].clone().detach()
                norm = torch.norm(vector)
                norm_vec = vector/norm
                dist,indices = index.search(norm_vec.cpu().numpy(),min(10,len(vector_base)))  # .view(1,-1)
                print(f"{metric_type}:  dist:{dist[0][:min(5,len(vector_base))]}   index: {indices}")
                similar_vec = []
                similar_vec.append(vector_base[indices[0][0]])
        else:pass # 找不到subject 就不检索
        handle.remove()
        handle2.remove()
        handle3.remove()
        handle4.remove()
        ...
        elif len(start_end)>0:
            if dist[0][0]>=threshold: searched_prefix = torch.tensor(similar_vec).to(device)  
            else: searched_prefix = None

        prefix = torch.arange(start=0,end=prefix_len).to(hparams.device) #
        prefix_list = []
        for i in range(batchsize):
            prefix_list.append(embd_list[i](prefix))
        prefix_embds = torch.cat(prefix_list,dim=0)

        new_past_kv = tuple()
        from ..prefix_editor.utils  import register_token_hook #,get_kv
        # if locality==True:
        #     searched_prefix = None
        tok_handle = None
        if searched_prefix is not None:
            tok_handle = register_token_hook(model,searched_prefix,hparams,device)
        # elif searched_prefix is None:
        #     new_past_kv = PAD_past_kv
        

        logits = model(input_ids=prompt_target_ids).logits
        # logits = model(input_ids=prompt_target_ids,past_key_values=PAD_past_kv).logits
        # logits = model(input_ids=prompt_target_ids,past_key_values=new_past_kv).logits
        if tok_handle:
            tok_handle.remove()
            # print(f"metric_type: {metric_type}  tok_handle")
        answers = torch.argmax(logits, dim=-1).squeeze().detach().cpu().numpy().tolist()
        labels = prompt_target_ids.squeeze().detach().cpu().numpy().tolist()
        # labels = [i for i in range(prefix_len)] + prompt_target_ids.squeeze().detach().cpu().numpy().tolist()

        answers = slice_list(answers,prompt_len,left=True)
        labels = slice_list(labels,prompt_len,left=False)
        # print(f"after answers: {answers}")
        # print(f"after labels: {labels}")
        
        # answers = answers[...,:-1,:]
        # labels = labels[...,1:,:]
        # if locality:
        #     return answers if type(answers[0]) is list else [answers,],dist,indices,threshold
        ...
        if isinstance(answers[0], list):
            ...
            return [np.mean(np.equal(answers, labels))],dist,indices,threshold
