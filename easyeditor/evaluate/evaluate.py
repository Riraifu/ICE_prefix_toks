...
from .portability_evaluate import compute_portability_quality,compute_portability_quality2
from .evaluate_utils import (
    test_seq2seq_batch_prediction_acc, 
    test_batch_prediction_acc, 
    test_prediction_acc,
    test_prediction_acc2,
    test_generation_quality, 
    test_generation_quality2,
    test_concept_gen,
    test_safety_gen,
...
def compute_edit_quality2(
    ...
    if isinstance(model,LORA):
        model=model.model
    # First, unpack rewrite evaluation record.
    # print(f"record: {requests}")
    # print(f"record: {len(requests)}")
    results = []
    for idx,record in enumerate(requests):
        ...
        rewrite_prompts = record["prompt"]
        rephrase_prompts = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
        # ret = compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,rewrite_prompts, target_new, device=device, eval_metric=eval_metric)
        
        ret = compute_rewrite_or_rephrase_quality2(embd_list,idx,model, model_name, hparams, tok,rewrite_prompts, target_new, device=device, eval_metric=eval_metric,index=index,index2=index2,vector_base=vector_base,requests=record,metric_type="Edit_Succ",trained_fusion_ids=trained_fusion_ids)
        # return ret
        results.append(ret)
        # continue
        ret['locality'] = {}
        ret['portability'] = {}
        # if rephrase_prompts is not None:
        #     # ret.update(compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,rephrase_prompts, target_new, device=device, test_rephrase=True, eval_metric=eval_metric))
        #     ret.update(compute_rewrite_or_rephrase_quality2(embd_list,idx,model, model_name, hparams, tok,rephrase_prompts, target_new, device=device, test_rephrase=True, eval_metric=eval_metric,index=index,index2=index2,vector_base=vector_base,metric_type="rephrase"))


        if 'portability' in record.keys() and any(record['portability']):
            for portability_key in record['portability'].keys():
                # ret['portability'].update(compute_portability_quality(model, model_name, hparams, tok, portability_key,record['portability'][portability_key]['prompt'],record['portability'][portability_key]['ground_truth'], device=device))
                ret['portability'].update(compute_portability_quality2(embd_list,idx,model, model_name, hparams, tok, portability_key,record['portability'][portability_key]['prompt'],record['portability'][portability_key]['ground_truth'], device=device,index=index,index2=index2,vector_base=vector_base,requests=record,metric_type="portability",trained_fusion_ids=trained_fusion_ids))

        if 'locality' in record.keys() and any(record['locality']):
            for locality_key in record['locality'].keys():
                # ret['locality'].update(compute_locality_quality(model, model_name, hparams, tok, locality_key,record['locality'][locality_key]['prompt'],record['locality'][locality_key]['ground_truth'], device=device))
                ret['locality'].update(compute_locality_quality2(embd_list,idx,model, model_name, hparams, tok, locality_key,record['locality'][locality_key]['prompt'],record['locality'][locality_key]['ground_truth'], device=device,index=index,index2=index2,vector_base=vector_base,metric_type="locality",trained_fusion_ids=trained_fusion_ids))
        print(f"ES_acc: {ret['rewrite_acc']}")
        print(f"GS_acc: {ret['portability']['Generality_acc']}")
        print(f"LS_acc: {ret['locality']['Locality_acc']}")
        continue

        # 似乎不需要加index   因为不跟最终指标直接关联？
        if test_generation:
            if hparams.alg_name == 'GRACE':
                ret['fluency'] = test_generation_quality(model=model,tok=tok,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=True)
            else:
                # ret['fluency'] = test_generation_quality(model=model,tok=tok,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=False)
                ret['fluency'] = test_generation_quality2(embd=embd_list[idx],hparams=hparams,model=model,tok=tok,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=False,device=device,index=index,vector_base=vector_base,metric_type="fluency")
...
def get_gen_sent2(
    ...
    if test_generation and save_gen_sentence:
        # sent = save_generation_sentence(model=model,tok=tok,target_new=target_new,ground_truth=ground_truth,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=False)
        sent = save_generation_sentence2(embd=embd,hparams=hparams,model=model,tok=tok,target_new=target_new,ground_truth=ground_truth,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=False,device=device,index=index,vector_base=vector_base,metric_type=metric_type)
    return sent 
...
# 重载 me
def compute_rewrite_or_rephrase_quality2(
    embd_list,
    idx,
    model,
    model_name,
    ...
    # model.transformer.h[hparams.n_layer-2].attn.start_end = []
    ...
    if 'gpt2' in hparams.M_name or 'gpt2-xl' in hparams.M_name or 'gpt-j-6b' in hparams.M_name:
        # _subject = [' '+requests['subject']]*batchsize
        # _subject_ids = tok(_subject, return_tensors="pt", padding=True)['input_ids'].to(device)
        # subject = [requests['subject']]*batchsize
        # subject_ids = tok(subject, return_tensors="pt", padding=True)['input_ids'].to(device)
        
        # subject_ids_list = [str(id) for id in _subject_ids[0].cpu().tolist() if id!=tok.pad_token_id]  # 仅支持batchsize=1
        # subject_ids_str = '_'.join(subject_ids_list)
        # trained_fusion_ids[subject_ids_str]=subject[0]
        
        
        '''ES_ACC  GS_ACC中必须存在subject  不存在就掉准确率 尽量保证不掉'''
        subject_ids,start_end = find_subject(prompt_ids, trained_fusion_ids)
        subject_ids = subject_ids.to(device)
        # start_end = get_start_end(tok,hparams.M_name,prompt_ids,subject_ids,None)
        # start_end = get_start_end(tok,hparams.M_name,prompt_ids,_subject_ids,subject_ids)
        
    elif 'llama-2-7b' in hparams.M_name:
        # _subject = [requests['subject']]*batchsize
        # _subject_ids = tok(_subject, return_tensors="pt", padding=True)['input_ids'].to(device)
        # start_end = get_start_end(tok,hparams.M_name,prompt_ids,_subject_ids,subject_ids=None)
        subject_ids,start_end = find_subject(prompt_ids, trained_fusion_ids)
        subject_ids = subject_ids.to(device)
    ...
    #     str_start_idx = prompt.find(_subject)
    #     # print(f"contexts_prompts_targets_str[i][:str_start_idx]:{contexts_prompts_targets_str[i][:str_start_idx]}")
    #     print(f"prompt[str_start_idx:]:{prompt[str_start_idx:]}")
    #     pre = tok(prompt[:str_start_idx], return_tensors='pt', padding=True)['input_ids'].to(device)
    #     _subject_pre = pre.shape[1]
    #     start_idx = _subject_pre #+ 1
    #     end_idx = start_idx + _subject_len - 1
    #     start_end.append([start_idx+prefix_len,end_idx+prefix_len])                      # 加上<pad>长度

        # print(f"batchsize: {batchsize}")
        # print(f"prompt: {prompt}")   # 加上<pad>长度
        # print(f"prompt_ids: {prompt_ids[i][start_idx:end_idx+1]}")   # 加上<pad>长度
    ...
    if eval_metric == 'ppl':
        ppl = PPL(model, tok, prompt, target_new, device)
        # acc = test_prediction_acc(model, tok, hparams, prompt, target_new, device)
        acc,dist,indices,threshold = test_prediction_acc2([embd_list[idx]],model, tok, hparams, prompt, target_new, device,index=index,index2=index2,vector_base=vector_base,metric_type=metric_type,start_end=start_end)
        ret = {
            f"{key}_ppl": ppl,
            f"{key}_acc": acc,
            f"{key}_dist": str(dist),
            f"{key}_indices": str(indices),
            f"{key}_threshold": threshold
        }
    # elif hparams.alg_name=="GRACE":
    #     # ppl = PPL(model, tok, prompt, target_new, device)
    #     if 't5' in model_name.lower():
...
def compute_rewrite_or_rephrase_quality3(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
...
def compute_locality_quality2(
    ...
    if 't5' in model_name.lower():
        ...
        # loc_tokens = test_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device, locality=True, vanilla_generation=hparams.alg_name=='GRACE')
        # loc_tokens,dist,indices,threshold = test_prediction_acc2([embd_list[idx]],model, tok, hparams, prompt, locality_ground_truth, device, locality=True, vanilla_generation=hparams.alg_name=='GRACE',index=index,vector_base=vector_base,metric_type=metric_type)
    ...
    acc,dist,indices,threshold = test_prediction_acc2([embd_list[idx]],model, tok, hparams, prompt, locality_ground_truth, device,locality=True,vanilla_generation=hparams.alg_name=='GRACE', index=index,index2=index2,vector_base=vector_base,metric_type=metric_type,start_end=start_end)
    
    ret = {
        f"{locality_key}_acc": acc,
        f"{locality_key}_dist": str(dist),
        f"{locality_key}_indices": str(indices),
        f"{locality_key}_threshold": threshold
    }
    # ret = {
    #     f"{locality_key}_output": loc_tokens,
    #     f"{locality_key}_dist": str(dist),
