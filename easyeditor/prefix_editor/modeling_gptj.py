...
from transformers.models.deprecated._archive_maps import GPTJ_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
class GPTJAttention2(GPTJAttention):
    def __init__(self, config):
    ...
    def _attn(
        ...
        for i in range(batch_size):
            ...
            causal_mask_clone[:,:,prefix_len:start,:prefix_len]=False 
        ...
        attn_weights = torch.where(causal_mask, attn_weights, mask_value)
        # attn_weights = torch.where(causal_mask, final_causal_mask, mask_value)
...
class GPTJBlock2(GPTJBlock):
    def __init__(self, config):
        super().__init__(config)
        inner_dim = config.n_inner if config.n_inner is not None else 4 * config.n_embd
        self.ln_1 = nn.LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        self.attn = GPTJAttention2(config)
        self.mlp = GPTJMLP(inner_dim, config)
...
class GPTJModel(GPTJPreTrainedModel):
    def __init__(self, config):
        ...
        # block_list = [GPTJBlock(config) for _ in range(config.n_layer)]
        block_list = [GPTJBlock(config) for _ in range(config.n_layer-1)] + [GPTJBlock2(config)]
